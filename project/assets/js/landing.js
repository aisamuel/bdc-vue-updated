    ...
    methods: {
      ...
      async addBankAccount() {
        ...
        const res = await fetch(
          `${baseURL}${this.addBankURL}${this.userProfile._id}`,
          {
            method: "POST",
            headers: {
      ...
      async fetchUserAccounts() {
        const res = await fetch(
          `${baseURL}${this.userBankAccountsURL}${this.userProfile._id}`,
          { mode: "cors" }
        );
        if (res) {
          const result = await res.json();
      ...
      convertCurrency(toNaira = true) {
        if (toNaira) {
          this.amountInNaira =
            this.selectedCurrency.rateToNaira * this.amountInCurrency;
        } else {
          this.amountInCurrency =
            this.amountInNaira / this.selectedCurrency.rateToNaira;
        }
      },
      switchMode(mode = "buy") {
      ...
      checkAccountIsUnique() {
        ...
        this.userBankAccounts.find(val => {
          console.log(val);
          if (
            Number(val.accountNumber) ===
            Number(this.userSelectedBankAccount.accountNumber)
          ) {
